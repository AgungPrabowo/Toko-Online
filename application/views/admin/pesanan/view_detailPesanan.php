...
<?php $this->load->view('admin/view_menuAtas');?>
<style type="text/css">
#wadah{
...
<div class="container-fluid" id="wadah">
	<div class="row">
		<div class="col-xs-12 col-sm-12 col-md-12 center-block"><!-- CONTENT -->
			...
			//ambil gambar di tabel produk dengan parameter judul/nama produk
			$gambar = $this->Model_produk->ambil_gambar($row->judul);?>
			<div class="row" style="backgound:red;" id="list-produk">
				...
				<div class="col-xs-8 col-sm-8 col-md-8 col-centered"><!-- Isi -->
					<p><strong><?=br(1).$row->judul;?></strong></p>
					Harga&nbsp:&nbsp
		   ...
		   <?php endforeach;?><hr>
			...
			<div class="row" id="sub-total">
			   ...
			   <!-- ambil data pembeli -->
			   <?php $query = $this->Model_pembeli->getNamaPembeli($order->row()->id_pembeli) ?>
			   <?=br(3);?><p class="text-center text-uppercase"><font color="#727272">Informasi Pengiriman</font><hr></p>
			   <b>Nama Penerima</b> 
			   <?=br(1).$query->nama_pembeli.br(2);?>

			   <b>Alamat Penerima</b> 
			   <?=br(1).$query->alamat.br(1).$query->kota.br(2);?>

			   <b>Telephone</b>
			   <?=br(1).$query->no_telp.br(2);?>

			   <b>Tanggal</b>
			   <?=br(1).generate_tanggal(gmdate('d/m/Y-H:i',$query->tgl_beli)).br(3);?>
